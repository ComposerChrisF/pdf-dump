...
3. **Lenient repair** (`recover.rs`) — after `Document::load`, `run()` recovers content streams lopdf dropped to bare dictionaries from a wrong `/Length` (with a loud stderr banner) before dispatch, so every mode sees the repaired document.  `--strict` inverts this: detect-only, no repair, exit 3.  When anything is recovered (or, under `--strict`, detected), a machine-readable `recovery` object is merged into every `--json` mode via `print_json_with_recovery`.
4. **Dispatch** (`lib.rs`) — `run()` matches on `ResolvedMode`:
...
| `helpers.rs` | Shared formatting utilities; `read_content_streams` (joins a page’s `/Contents` segments with a newline so tokens never fuse); `inherited_page_attr` (walks `/Parent` for `MediaBox`/`CropBox`/`Rotate`) |
