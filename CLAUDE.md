...
3. **Lenient repair** (`recover.rs`) — after `Document::load`, `run()` recovers content streams lopdf dropped to bare dictionaries from a wrong `/Length` (with a loud stderr banner) before dispatch, so every mode sees the repaired document.
...
| `recover.rs` | Lenient post-load repair: `recover_malformed_streams` promotes objects lopdf dropped to bare dictionaries (because the PDF declared a wrong `/Length`, so `endstream` wasn’t where the length said) back to `Object::Stream`, scanning the raw bytes to the `endstream` keyword via each object’s xref offset.  `has_candidates` gates the re-read; `recovery_banner` prints the loud stderr warning (declared vs. actual length, file offset).  Run once in `run()` after `Document::load` |
