...
The entire tool lives in `src/main.rs` (~1600 lines + ~2500 lines of tests). The flow is:

1. **CLI parsing** — `Args` struct via clap derive. Modes: dump (default), extract, inspect object, summary, page, metadata, search, text, refs-to, fonts, images, validate. Only one mode flag at a time (with exceptions: `--search --summary`, `--text --page`).
...
15. **JSON modifier** (`--json`) — Structured JSON output for all modes. Uses `serde_json`. Each PDF object maps to a JSON type schema.
16. **`print_object`** — Recursive pretty-printer that handles all `lopdf::Object` variants. Collects `(is_contents, ObjectId)` pairs into `child_refs` for the caller to traverse. When a dictionary key is `/Contents`, the `is_contents` flag propagates so content streams get parsed via `lopdf::content::Content::decode`.
17. **`decode_stream`** — Checks `/Filter` for `FlateDecode` and decompresses with `flate2::ZlibDecoder`. Returns `Cow<[u8]>` (borrowed if no decompression needed).
18. **`object_to_json`** — Maps each `lopdf::Object` variant to a `serde_json::Value` with a `type` field + value fields.
