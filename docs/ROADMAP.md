...
- **Standard-14 passthrough.**  A bare `Helvetica`/`Times-Roman`/`Courier` with no `/Encoding` and no `/ToUnicode` is reported Reliable but decoded by raw byte passthrough — only ASCII is accurate.  Its builtin StandardEncoding makes `0x27`/`0x60` the curly quotes ’/‘ and the high range accented/ligature glyphs, all of which passthrough mangles.  Anchored in `classify_passthrough`; a fix would route these through the `standard` table.
